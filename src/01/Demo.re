...
let mass_to_fuel = (mass) => mass / 3 - 2;


let test = (num) => {
  print_endline("testing");
  print_endline("mass: " ++ string_of_int(num));
  print_endline("fuel: " ++ string_of_int(mass_to_fuel(num)));
}
...
let input_masses = slurp_file(input_file);

let sum = List.fold_left(
  (acc, current_mass) => acc + mass_to_fuel(int_of_string(current_mass)),
  0,
  input_masses
);
...
any, is instead handled by wishing really hard, which has no mass and is outside
...
let mass_to_fuel2 = (mass) => {
  let rec helper = (current_mass, acc) => {
    let current_fuel = current_mass / 3 - 2;
    if (current_fuel <= 0) {
     acc
    } else {
      helper(current_fuel, current_fuel + acc)
    }
  }
    helper(mass, 0)
};
...
let test2 = (num) => {
  print_endline("testing");
  print_endline("mass: " ++ string_of_int(num));
  print_endline("fuel: " ++ string_of_int(mass_to_fuel2(num)));
};
...
let sum2 = List.fold_left(
  (acc, current_mass) => acc + mass_to_fuel2(int_of_string(current_mass)),
  0,
  input_masses
);

print_endline(string_of_int(sum2));
